...
- **All `common/src/smartphone/*/` lessons (sm_game):** every lesson whose content is loaded from `common/src/smartphone/{en-US,pt-BR,es-ES}/<uuid>.json` and is launched from a hub must have this button. A lesson group is the set of Android screens produced from one smartphone JSON `<uuid>`; the button belongs on the group's final content screen (its `Conclusion`/`Decision` screen, i.e. the screen that uses the JSON's last content entry). This applies to every hub (First Steps (Counting), I Am (Not) Like You, Workout, Abacus, Yupana, Road to Somewhere, Running and Growing, Where Are We From, Another Way to Count), and because the content is shared across locales, add/verify it for all three languages at once. Interactive games launched from a hub (e.g. Sequence Game, Building Game) and single-screen exercises keep their existing back navigation and are out of scope for this rule.
