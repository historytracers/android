...
- Current section lists to keep in sync:
  ...
  - **Another Way to Count** (8): `quipus`, `practicing_with_quipus`, `mesoamerican_symbols`, `overcoming_limits`, `building_like_a_mesoamerican`, `mesoamerican_orders`, `text_or_number`, `missing_numbers`
...
- Current list (latest first): `missing_numbers` (The Missing Numbers), `text_or_number` (Text or Number), `mesoamerican_orders` (Mesoamerican Orders), `building_like_a_mesoamerican` (Building Like a Mesoamerican), `overcoming_limits` (Overcoming Limits).
